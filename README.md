...
- **Backpressure**: If you see log messages like `"flush skipped due to backpressure"`, the server is responding slower than your send rate. With the default `BackpressureRequeue` policy, data is re-queued (not lost). If the buffer exceeds `maxBufferPoints`, oldest data is dropped to prevent unbounded memory growth. Consider increasing `WithBatchSize` to reduce request overhead, or increase `WithMaxConcurrentFlushes` if your network can handle more parallel requests.
